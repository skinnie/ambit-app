...
function main() {
  ...
  const variant = require(path.join(LINK, 'variant.js'));
  ...
  // this stuff device per device when a new device is added."
  //
  // SuuntoLink answers all of it - getMaxSportModes/getMaxDisplays/getMaxSuuntoApps plus a
  // supports*() per feature - so this asks for every variant it knows rather than encoding
  // one watch's answers. A device this project has never seen still gets the right UI.
  //
  // It confirms what André already knew (Traverse and Traverse Alpha have no multisport;
  // 5 modes and 4 displays rather than 10 and 8) and adds one he did not: the Ambit3 RUN
  // has no multisport either - only Peak, Sport and Vertical do.
  const CAPABILITIES = [
    'supportsMultisportModes', 'supportsDisplay', 'supportsAutoscroll',
    'supportsIntervalTimer', 'supportsQuickNavigation', 'supportsAltiBaroProfile',
    'supportsGpsAccuracy', 'supportsHeartRateLimits', 'supportsBacklightMode',
    'supportsFootPod', 'supportsBikePod', 'supportsPowerPod', 'supportsCadencePod',
  ];
  const variants = {};
  for (const [name, code] of Object.entries(variant.Variant)) {
    if (!Number.isNaN(Number(name))) continue;
    let row;
    try {
      row = {
        name,
        maxSportModes: sm.getMaxSportModes(code),
        maxDisplays: sm.getMaxDisplays(code),
        maxSuuntoApps: sm.getMaxSuuntoApps(code),
        // SuuntoLink caps multisport modes at 2 in its own UI ("Create multisport mode
        // 1/2 used", André's screenshot). The module exposes no getter for it, so the
        // number is recorded here from that screen rather than invented.
        maxMultisportModes: sm.supportsMultisportModes(code) ? 2 : 0,
      };
    } catch (e) {
      continue;                       // not a watch this module answers for
    }
    for (const cap of CAPABILITIES) {
      try { row[cap.replace('supports', 'supports')] = !!sm[cap](code); }
      catch (e) { row[cap] = false; }
    }
    variants[code] = row;             // keyed by the codename our tools already use
  }

  // Friendly names, from SuuntoLink's own Devices.xml - <device name="Suunto Ambit3 Peak"
  // type=".." model="Emu"> - so the picker in Testing mode names devices the way Suunto
  // does rather than showing internal codenames. Several entries share a model (different
  // USB pids of the same watch); the first name wins, and the "BSL" bootloader variants are
  // skipped since they are the same watch in a different state.
  const xml = fs.readFileSync(path.join(LINK, 'Devices.xml'), 'utf8');
  for (const m of xml.matchAll(/<device name="([^"]+)" type="[^"]+" model="([^"]+)"/g)) {
    const [, name, model] = m;
    if (name.endsWith(' BSL')) continue;
    if (variants[model] && !variants[model].productName)
      variants[model].productName = name;
  }
